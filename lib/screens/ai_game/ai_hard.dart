...
class AiHardGame extends StatefulWidget {
  const AiHardGame({super.key});

  @override
  State<AiHardGame> createState() => _AiHardGameState();
}

class _AiHardGameState extends State<AiHardGame>
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < gridSize; i++) {
      for (int j = 0; j < gridSize; j++) {
        if (boxes[i][j] != 0) {
          paint.color =
              (boxes[i][j] == 1 ? Colors.cyan : Colors.pink).withOpacity(0.25);
          canvas.drawRect(
            Rect.fromLTWH(
              offset + j * spacing,
              offset + i * spacing,
